...
class Motor:
    """A servo and the pulley it drives."""
    ...
    def force_n(self) -> float:
        """Belt force this motor can hold at its configured torque limit."""
        return self.torque_nm / self.pulley_radius_m

    @property
    def reflected_mass_kg(self) -> float:
        """The rotor seen from the belt: J/r². On a short-rotation-distance
        pulley this dominates a light gantry, which is why the drive manual
        warns about load-to-rotor inertia ratio at all."""
        return self.rotor_inertia_kgm2 / (self.pulley_radius_m**2)

    @property
    def free_speed_mm_s(self) -> float:
        return self.max_rpm / 60.0 * self.rotation_distance_mm

    @property
    def rated_speed_mm_s(self) -> float:
        return self.rated_rpm / 60.0 * self.rotation_distance_mm
...
def slot_torques_nm(machine: Machine, axis_accel_mm_s2):
    ...
    for slot in range(2):
        ...
        rotor = (
            machine.motor.rotor_inertia_kgm2
            * slot_accel[slot]
            / machine.motor.pulley_radius_m
        )
        torques.append(belt_force * machine.motor.pulley_radius_m + rotor)
    return torques
...
def max_axis_accel_mm_s2(machine: Machine, direction) -> float:
    ...
    if worst == 0.0:
        return math.inf
    return 1000.0 * machine.motor.torque_nm / worst
...
def max_axis_velocity_mm_s(machine: Machine, direction) -> float:
    ...
    if fastest == 0.0:
        return math.inf
    return machine.motor.free_speed_mm_s / fastest
...
def inertia_ratio(machine: Machine) -> float:
    ...
    heaviest = max(machine.mode_mass_kg())
    return heaviest / machine.motor.reflected_mass_kg
